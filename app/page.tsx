...
export default async function Home(){
 ...
 return <><PublicHeader/><main className="container page-space"><VisitTracker/><section className="hero"><div className="eyebrow">WELCOME TO</div><h1>NIGHTTOONS INDIA</h1><p>Hindi Dubbed Anime in One Place</p><div className="hero-actions"><Link className="btn" href="/search">🔎 Search Anime</Link><Link className="btn secondary" href="/wishlist">♡ Wishlist</Link></div></section><section><div className="section-head"><div><h2>Latest Anime</h2><p className="muted">Freshly added titles</p></div><Link href="/search">View all →</Link></div>{animeList.length?<div className="anime-grid">{animeList.map((a:any)=><AnimeCard anime={a} key={a.id} variant="home" />)}</div>:<div className="card empty"><h2>No anime available yet</h2><p className="muted">Add your first anime from the Owner Dashboard.</p></div>}</section><TelegramCards/></main></>
}
